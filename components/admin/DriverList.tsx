...
export default function DriversList() {
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'On Route': return 'bg-blue-500 text-blue-600';
      case 'Idle': return 'bg-amber-500 text-amber-600';
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {filteredDrivers.map((driver) => (
          ...
          >
            <div className={`h-1.5 w-full ${getStatusStyle(driver.status).split(' ')[0]}`} />
            
            <div className="p-5 flex flex-col h-full">
              <div className="flex justify-between items-start mb-4">
                <span className={`text-[8px] font-black px-2.5 py-1 rounded-lg uppercase tracking-wider bg-opacity-10 ${getStatusStyle(driver.status)} bg-current`}>
                  {driver.status}
                </span>
                <span className="text-[10px] text-slate-400 font-bold font-mono uppercase">{driver.truck}</span>
              </div>
              ...
              <div className="flex items-center justify-between p-3.5 bg-slate-50 rounded-xl border border-slate-100 mb-2">
                <div>
                  <span className="text-[8px] font-black text-slate-400 uppercase block mb-0.5">Efficiency Score</span>
                  <span className="text-xl font-black text-slate-900 leading-none">{driver.efficiency}%</span>
                </div>
                <div className="w-10 h-10 rounded-lg bg-white border border-slate-100 flex items-center justify-center text-lg shadow-sm">🚚</div>
              </div>
            </div>
          </div>
        ))}
      ...
      {selectedDriver && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-4 duration-500 ease-out">
            ...
            <div className={`h-1.5 w-full ${getStatusStyle(selectedDriver.status).split(' ')[0]}`} />
            
            <div className="p-6 md:p-8">
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-2xl shadow-inner">🪪</div>
                  <div>
                    <h2 className="text-xl font-black text-slate-900 tracking-tight leading-none mb-1">{selectedDriver.name}</h2>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedDriver.id} • Operator</p>
                  </div>
                </div>
                <button onClick={() => setSelectedDriver(null)} className="p-2 bg-slate-50 text-slate-400 hover:text-slate-900 rounded-lg transition-colors">✕</button>
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                    ...
                    <p className="text-xs font-bold text-slate-800 font-mono">{selectedDriver.truck}</p>
                  </div>
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Fleet Score</p>
                    <p className="text-xs font-black text-emerald-600">{selectedDriver.efficiency}% Performance</p>
                  </div>
                </div>

                <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 flex justify-between items-center group">
                  <div>
                    <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Phone Connection</p>
                    <p className="text-xs font-bold text-slate-800">{selectedDriver.phone}</p>
                  </div>
                  <span className="text-lg">📞</span>
                </div>

                <div className="p-5 bg-slate-900 rounded-2xl relative overflow-hidden text-white">
                  <div className="absolute right-[-5px] top-[-5px] opacity-10 text-4xl">📋</div>
                  <p className="text-[9px] font-black text-slate-500 uppercase mb-2 tracking-widest">Driver Credentials</p>
                  <p className="text-xs font-mono font-black tracking-[0.2em]">{selectedDriver.license}</p>
                </div>
