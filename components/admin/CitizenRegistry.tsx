          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-50 bg-slate-50/30">
                ...
                <th className="hidden md:table-cell px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Barangay
                </th>
                ...
                >
                  <td className="px-6 md:px-8 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-xs border border-white shadow-sm">
                        👤
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-2 h-2 rounded-full ${citizen.violations > 2 ? "bg-red-500" : citizen.violations > 0 ? "bg-amber-500" : "bg-emerald-500"} animate-pulse`}
                      />
                      <span
                        className={`text-[10px] font-black ${citizen.violations > 0 ? "text-red-500" : "text-emerald-500"}`}
                      >
                        {citizen.violations}{" "}
                        {citizen.violations === 1 ? "Violation" : "Violations"}
                      </span>
                    </div>
                  </td>
      ...
      {selectedCitizen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-full sm:slide-in-from-bottom-4 duration-500 ease-out">
            <div
              className={`h-1.5 w-full ${selectedCitizen.violations > 0 ? "bg-red-500" : "bg-emerald-500"}`}
            />
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-xl bg-slate-50 flex items-center justify-center text-2xl border border-slate-100">
                    👤
                  </div>
                  <div>
                ...
                <button
                  onClick={() => setSelectedCitizen(null)}
                  className="p-2 bg-slate-50 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-all active:scale-90"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="space-y-4">
                ...
                {/* Status Banner */}
                <div
                  className={`p-5 rounded-xl border ${selectedCitizen.violations > 0 ? "bg-red-50/50 border-red-100" : "bg-emerald-50/50 border-emerald-100"}`}
                >
                  <p
                    className={`text-[10px] font-black uppercase mb-1 tracking-widest ${selectedCitizen.violations > 0 ? "text-red-700" : "text-emerald-700"}`}
                  >
                    Compliance Summary
                  </p>
                  <p
                    className={`text-sm font-black tracking-tight ${selectedCitizen.violations > 0 ? "text-red-900" : "text-emerald-900"}`}
                  >
                    {selectedCitizen.violations === 0
                      ? "✓ Registered Eco-Champion"
                      : `⚠️ ${selectedCitizen.violations} Violations Tracked`}
                  </p>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    className="flex-[1.5] py-4 bg-slate-900 text-white rounded-xl font-black text-[10px] tracking-widest uppercase shadow-lg shadow-slate-200 hover:bg-emerald-600 transition-all active:scale-95"
                    onClick={() => {
                      /* Contact logic */
                    }}
                  >
