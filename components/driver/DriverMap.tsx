...
export default function DriverMap() {
  ...
  useEffect(() => {
    ...
    const active = bins.filter((b) => b.fillLevel >= 40 || b.id === selectedBinId);
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full bg-slate-50 overflow-hidden relative">
      ...
      <NavigationControls isEditMode={isEditMode} setIsEditMode={setIsEditMode} isNavMode={isNavMode} setIsNavMode={setIsNavMode} heading={heading} />

      <div className="absolute inset-0 md:relative md:flex-1 h-full overflow-hidden order-1">
        <MapContainer center={LUPON_CENTER} zoom={18} maxZoom={22} zoomControl={false} style={{ height: "100%", width: "100%" }}>
          <TileLayer url={`https://api.mapbox.com/styles/v1/mapbox/outdoors-v12/tiles/{z}/{x}/{y}?access_token=${process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN}`} attribution="© Mapbox" />
        ...
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="hidden md:flex absolute top-1/2 right-4 z-[1002] -translate-y-1/2 w-8 h-12 bg-white border border-slate-200 shadow-xl rounded-xl items-center justify-center">{isSidebarOpen ? "❯" : "❮"}</button>
      </div>

      <div className={`fixed md:relative bottom-0 left-0 right-0 z-[1001] transition-all duration-500 ${isDashboardVisible ? "translate-y-0" : "translate-y-[calc(100%-48px)]"} ${isSidebarOpen ? "md:w-[400px]" : "md:w-0"} md:translate-y-0 md:h-full pointer-events-none md:pointer-events-auto order-2`}>
        <div className="pointer-events-auto h-full bg-white/95 backdrop-blur-md rounded-t-[3.5rem] md:rounded-none shadow-xl border-t md:border-t-0 md:border-l border-slate-100 flex flex-col overflow-hidden">
          <button className="w-full flex justify-center p-4 md:hidden" onClick={() => setIsDashboardVisible(!isDashboardVisible)}>
            <div className={`w-12 h-1.5 rounded-full ${isDashboardVisible ? "bg-slate-200" : "bg-emerald-500 w-16"}`} />
          </button>
          <div className="flex-1 overflow-y-auto">
            <EcoDashboard
              ...
              onRefresh={() => { setRouteKey((k) => k + 1); setSelectedBinId(null); }}
            />
          </div>
        </div>
      </div>
