...
export default function CitizenRegister() {
  const [formData, setFormData] = useState({
    ...
    address: "",
    houseLotNumber: "",
    serviceType: "General", // Added relevant app context
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Citizen Data:", formData);
  };
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border border-emerald-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <select 
              required
              className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:bg-white outline-none transition-all appearance-none"
          ...
          {/* App Context: Service Type */}
          <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100">
            <label className="block text-xs font-bold text-emerald-800 uppercase tracking-wider mb-2">Primary Collection Type</label>
          ...
          <button className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-emerald-200 hover:bg-emerald-700 active:scale-[0.98] transition-all">
            Create EcoRoute Account
          </button>
        </form>
