...
export default function Page() {
  ...
  useEffect(() => {
    ...
    setStaffPage(1);
    setCitizenPage(1);
  }, [search, activeTab, roleFilter, citizenFilter]);
  ...
  const markRead=async(id:string)=>{
    ...
    setNotifs(p=>p.map(item=>item.id===id?{...item,is_read:true}:item));
  ...
  const totalStaffPages = Math.ceil(filteredUsers.length / STAFF_PAGE_SIZE);
  const paginatedUsers = filteredUsers.slice((staffPage - 1) * STAFF_PAGE_SIZE, staffPage * STAFF_PAGE_SIZE);
  ...
  const totalCitizenPages = Math.ceil(filteredCitizens.length / CITIZEN_PAGE_SIZE);
  const paginatedCitizens = filteredCitizens.slice((citizenPage - 1) * CITIZEN_PAGE_SIZE, citizenPage * CITIZEN_PAGE_SIZE);
  const filteredAuditLogs=auditLogs.filter(l=>search===""||l.action_type.toLowerCase().includes(search.toLowerCase())||(l.admin_name??"").toLowerCase().includes(search.toLowerCase())||(l.reason??"").toLowerCase().includes(search.toLowerCase()));
  const totalAuditPages = Math.ceil(filteredAuditLogs.length / AUDIT_PAGE_SIZE);
  const paginatedAuditLogs = filteredAuditLogs.slice((auditPage - 1) * AUDIT_PAGE_SIZE, auditPage * AUDIT_PAGE_SIZE);
        ...
        <header className="h-16 md:h-20 px-4 md:px-8" style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,zIndex:100,position:"relative"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16,marginLeft:"auto"}}>
            ...
            <div ref={notifRef} style={{position:"relative"}}>
              ...
              {notifOpen&&(<div className="notif-dropdown" style={{position:"absolute",top:"calc(100% + 12px)",right:0,width:340,background:"#ffffff",borderRadius:16,border:"1px solid #e5e7eb",boxShadow:"0 20px 40px rgba(0,0,0,.08)",zIndex:300,animation:"dropIn .2s ease both",overflow:"hidden"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px",borderBottom:"1px solid #f3f4f6",background:"#f9fafb"}}><div style={{display:"flex",flexDirection:"column",gap:2}}><span style={{fontSize:14,fontWeight:700,color:"#111827"}}>Notifications</span>{unreadC>0&&<button onClick={markAllRead} style={{background:"none",border:"none",padding:0,color:"#1c4532",fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"left",textTransform:"uppercase",letterSpacing:".02em"}}>Mark all as read</button>}</div><button onClick={()=>setNotifOpen(false)} style={{background:"none",border:"none",cursor:"pointer",display:"flex"}}><X size={16} color="#6b7280"/></button></div><div style={{maxHeight:320,overflowY:"auto"}}>{notifs.length===0?<div style={{padding:32,textAlign:"center",color:"#6b7280",fontSize:13}}>No notifications</div>:notifs.map(n=>(<div key={n.id} onClick={()=>markRead(n.id)} style={{padding:"14px 16px",borderBottom:"1px solid #f3f4f6",background:n.is_read?"#fff":"#f8fafc",cursor:"pointer",display:"flex",gap:12,alignItems:"flex-start"}}><div style={{width:32,height:32,borderRadius:"50%",background:n.is_read?"#f3f4f6":"#e0e7ff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Info size={14} color={n.is_read?"#9ca3af":"#4f46e5"}/></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:n.is_read?500:700,color:n.is_read?"#6b7280":"#111827"}}>{n.title}</div><div style={{fontSize:12,color:n.is_read?"#9ca3af":"#4b5563",marginTop:4,lineHeight:1.4}}>{n.body}</div><div style={{fontSize:11,color:"#9ca3af",marginTop:6}}>{timeAgo(n.created_at)}</div></div>{!n.is_read&&<div style={{width:8,height:8,borderRadius:"50%",background:"#3b82f6",flexShrink:0,marginTop:6}}/>}</div>))}</div></div>)}
            </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          <div className="max-w-[1400px] mx-auto p-3 md:p-8">
            ...
            <div style={{background:"#ffffff",borderRadius:24,border:"1px solid #e5e7eb",overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,0.02)"}}>
              ...
              {activeTab==="users"&&(<div style={{padding:"24px"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:16}}><div style={{display:"flex",alignItems:"center",gap:16,flex:1}}><select value={roleFilter} onChange={e=>setRoleFilter(e.target.value)} style={{fontSize:13,padding:"10px 14px",border:"1px solid #d1d5db",borderRadius:12,background:"#f9fafb",color:"#111827",outline:"none",cursor:"pointer"}}><option value="all">All roles</option><option value="SUPER_ADMIN">Super Admin</option><option value="ADMIN">Admin</option><option value="DRIVER">Driver</option><option value="LGU">LGU</option></select><div style={{position:"relative",flex:1,maxWidth:300}}><Search size={14} style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:"#6b7280"}}/><input placeholder="Search users…" value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",paddingLeft:38,paddingRight:14,paddingTop:10,paddingBottom:10,border:"1px solid #d1d5db",borderRadius:12,fontSize:13,color:"#111827",outline:"none",background:"#f9fafb",boxSizing:"border-box"}}/></div></div><div style={{fontSize:12,color:"#6b7280",fontWeight:500}}>Showing {(staffPage-1)*STAFF_PAGE_SIZE + 1} - {Math.min(staffPage*STAFF_PAGE_SIZE, filteredUsers.length)} of {filteredUsers.length} staff</div></div><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:800}}><thead><tr style={{borderBottom:"2px solid #e5e7eb"}}>{["User","Role","Detail","Status","Updated","Actions"].map(h=><th key={h} style={{padding:"14px 16px",textAlign:"left",fontSize:11,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:".05em"}}>{h}</th>)}</tr></thead><tbody>{paginatedUsers.length===0?<tr><td colSpan={6} style={{textAlign:"center",padding:60,color:"#6b7280",fontSize:14}}>No staff found</td></tr>:paginatedUsers.map(u=>(<tr key={u.id} className="row-hover" style={{borderBottom:"1px solid #f3f4f6"}}><td style={{padding:"16px"}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:40,height:40,borderRadius:"50%",background:ROLE_CONFIG[u.role]?.bg??"#f3f4f6",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:ROLE_CONFIG[u.role]?.color??"#1c4532",flexShrink:0}}>{(u.full_name??"?").charAt(0).toUpperCase()}</div><div><div style={{fontSize:14,fontWeight:600,color:u.is_archived?"#9ca3af":"#111827",textDecoration:u.is_archived?"line-through":"none",display:"flex",alignItems:"center",gap:8}}>{u.full_name??"—"}{u.id===meId&&<span style={{fontSize:10,fontWeight:700,color:"#fff",background:"#1c4532",padding:"2px 8px",borderRadius:20}}>YOU</span>}</div><div style={{fontSize:12,color:"#6b7280",marginTop:2}}>{u.email}</div></div></div></td><td style={{padding:"16px"}}><RoleBadge role={u.role}/></td><td style={{padding:"16px",fontSize:13,color:"#4b5563"}}>{u.role==="DRIVER"&&<div><div style={{color:"#111827",fontWeight:600}}>{u.license_number??"No license"}</div>{u.vehicle_plate_number&&<div style={{fontSize:12,marginTop:2}}>{u.vehicle_plate_number}</div>}<div style={{display:"flex",alignItems:"center",gap:6,marginTop:6}}><span style={{width:6,height:6,borderRadius:"50%",background:u.duty_status==="ON-DUTY"?"#10b981":"#9ca3af",display:"inline-block"}}/><span style={{fontSize:12,color:u.duty_status==="ON-DUTY"?"#059669":"#6b7280",fontWeight:500}}>{u.duty_status??"OFF-DUTY"}</span></div></div>}{u.role==="LGU"&&(u.barangay?<div><div style={{fontWeight:600,color:"#111827"}}>{u.barangay}</div><div style={{fontSize:12,marginTop:2}}>{u.position_title??"—"}</div>{u.municipality&&<div style={{fontSize:12,color:"#6b7280",marginTop:2}}>{u.municipality}</div>}</div>:<button onClick={()=>setAssignTarget(u)} style={{display:"inline-flex",alignItems:"center",gap:6,fontSize:12,fontWeight:600,padding:"6px 12px",borderRadius:20,cursor:"pointer",background:"#fef3c7",color:"#d97706",border:"none"}}><MapPin size={12}/>Assign LGU</button>)}{u.role==="ADMIN"&&<span style={{color:"#4b5563"}}>Barangay Admin</span>}{u.role==="SUPER_ADMIN"&&<span style={{color:"#1c4532",fontWeight:700}}>Full system access</span>}</td><td style={{padding:"16px"}}><span style={{fontSize:12,fontWeight:600,padding:"4px 12px",borderRadius:20,background:u.is_archived?"#f1f5f9":"#e6f0eb",color:u.is_archived?"#64748b":"#1c4532"}}>{u.is_archived?"Archived":"Active"}</span></td><td style={{padding:"16px",fontSize:12,color:"#6b7280",whiteSpace:"nowrap"}}>{timeAgo(u.updated_at)}</td><td style={{padding:"16px"}}>{processing===u.id?<div style={{width:20,height:20,borderRadius:"50%",border:"2px solid #e5e7eb",borderTopColor:"#1c4532",animation:"spin .8s linear infinite"}}/>:<ActionMenu user={u} meId={meId} onArchive={toggleArchive} onRole={changeRole} onAssign={setAssignTarget} onNotify={setNotifyTarget}/>}</td></tr>))}</tbody></table></div>{totalStaffPages>1&&(<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:24,padding:"16px 0",borderTop:"1px solid #f3f4f6"}}><button onClick={()=>setStaffPage(p=>Math.max(1,p-1))} disabled={staffPage===1} style={{padding:"8px 16px",borderRadius:10,border:"1px solid #e5e7eb",background:staffPage===1?"#f9fafb":"#fff",color:staffPage===1?"#9ca3af":"#4b5563",fontSize:13,fontWeight:600,cursor:staffPage===1?"not-allowed":"pointer"}}>Previous</button><div style={{display:"flex",alignItems:"center",gap:4}}>{Array.from({length:Math.min(5,totalStaffPages)},(_,i)=>{let pNum=i+1;if(totalStaffPages>5&&staffPage>3){pNum=staffPage-2+i;if(pNum+2>totalStaffPages)pNum=totalStaffPages-4+i;}if(pNum<=0||pNum>totalStaffPages)return null;return(<button key={pNum} onClick={()=>setStaffPage(pNum)} style={{width:36,height:36,borderRadius:10,border:"none",background:staffPage===pNum?"#1c4532":"transparent",color:staffPage===pNum?"#fff":"#4b5563",fontSize:13,fontWeight:700,cursor:"pointer"}}>{pNum}</button>);})}</div><button onClick={()=>setStaffPage(p=>Math.min(totalStaffPages,p+1))} disabled={staffPage===totalStaffPages} style={{padding:"8px 16px",borderRadius:10,border:"1px solid #e5e7eb",background:staffPage===totalStaffPages?"#f9fafb":"#fff",color:staffPage===totalStaffPages?"#9ca3af":"#4b5563",fontSize:13,fontWeight:600,cursor:staffPage===totalStaffPages?"not-allowed":"pointer"}}>Next</button></div>)}</div>)}
              
              {activeTab==="citizens"&&(<div style={{padding:"24px"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,flexWrap:"wrap",gap:16}}><div style={{display:"flex",alignItems:"center",gap:16,flex:1}}><select value={citizenFilter} onChange={e=>setCitizenFilter(e.target.value)} style={{fontSize:13,padding:"10px 14px",border:"1px solid #d1d5db",borderRadius:12,background:"#f9fafb",color:"#111827",outline:"none",cursor:"pointer"}}><option value="all">All citizens</option><option value="warnings">With warnings</option><option value="violations">With violations</option><option value="archived">Archived</option></select><div style={{position:"relative",flex:1,maxWidth:300}}><Search size={14} style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:"#6b7280"}}/><input placeholder="Search citizens…" value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",paddingLeft:38,paddingRight:14,paddingTop:10,paddingBottom:10,border:"1px solid #d1d5db",borderRadius:12,fontSize:13,color:"#111827",outline:"none",background:"#f9fafb",boxSizing:"border-box"}}/></div><div style={{fontSize:12,color:"#6b7280",fontWeight:500}}>Showing {(citizenPage-1)*CITIZEN_PAGE_SIZE + 1} - {Math.min(citizenPage*CITIZEN_PAGE_SIZE, filteredCitizens.length)} of {filteredCitizens.length}</div></div><div style={{display:"flex",gap:16,marginBottom:20}}>{[{label:"Total",val:citizens.length,color:"#1c4532"},{label:"Warnings",val:citizens.filter(c=>c.warning_count>0).length,color:"#d97706"},{label:"Violations",val:citizens.filter(c=>(c.violations?.length??0)>0).length,color:"#dc2626"}].map(s=>(<div key={s.label} style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:16,fontWeight:800,color:s.color}}>{s.val}</span><span style={{fontSize:12,fontWeight:600,color:"#6b7280",textTransform:"uppercase"}}>{s.label}</span></div>))}</div></div><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:900}}><thead><tr style={{borderBottom:"2px solid #e5e7eb"}}>{["Citizen","Barangay","Purok / Street","LGU Assigned","Warnings","Score","Violations","Status","Actions",""].map(h=><th key={h} style={{padding:"14px 16px",textAlign:"left",fontSize:11,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:".05em"}}>{h}</th>)}</tr></thead><tbody>{paginatedCitizens.length===0?<tr><td colSpan={10} style={{textAlign:"center",padding:60,color:"#6b7280",fontSize:14}}>No citizens found</td></tr>:paginatedCitizens.map(c=><CitizenRow key={c.id} c={c} lguMap={lguMap} onRole={changeRole}/>)}</tbody></table></div>{totalCitizenPages>1&&(<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:24,padding:"16px 0",borderTop:"1px solid #f3f4f6"}}><button onClick={()=>setCitizenPage(p=>Math.max(1,p-1))} disabled={citizenPage===1} style={{padding:"8px 16px",borderRadius:10,border:"1px solid #e5e7eb",background:citizenPage===1?"#f9fafb":"#fff",color:citizenPage===1?"#9ca3af":"#4b5563",fontSize:13,fontWeight:600,cursor:citizenPage===1?"not-allowed":"pointer"}}>Previous</button><div style={{display:"flex",alignItems:"center",gap:4}}>{Array.from({length:Math.min(5,totalCitizenPages)},(_,i)=>{let pNum=i+1;if(totalCitizenPages>5&&citizenPage>3){pNum=citizenPage-2+i;if(pNum+2>totalCitizenPages)pNum=totalCitizenPages-4+i;}if(pNum<=0||pNum>totalCitizenPages)return null;return(<button key={pNum} onClick={()=>setCitizenPage(pNum)} style={{width:36,height:36,borderRadius:10,border:"none",background:citizenPage===pNum?"#1c4532":"transparent",color:citizenPage===pNum?"#fff":"#4b5563",fontSize:13,fontWeight:700,cursor:"pointer"}}>{pNum}</button>);})}</div><button onClick={()=>setCitizenPage(p=>Math.min(totalCitizenPages,p+1))} disabled={citizenPage===totalCitizenPages} style={{padding:"8px 16px",borderRadius:10,border:"1px solid #e5e7eb",background:citizenPage===totalCitizenPages?"#f9fafb":"#fff",color:citizenPage===totalCitizenPages?"#9ca3af":"#4b5563",fontSize:13,fontWeight:600,cursor:citizenPage===totalCitizenPages?"not-allowed":"pointer"}}>Next</button></div>)}</div>)}
              
              {activeTab==="audit"&&(<div style={{padding:"24px"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}><div style={{maxWidth:300,position:"relative",flex:1}}><Search size={14} style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",color:"#6b7280"}}/><input placeholder="Search logs…" value={search} onChange={e=>setSearch(e.target.value)} style={{width:"100%",paddingLeft:38,paddingRight:14,paddingTop:10,paddingBottom:10,border:"1px solid #d1d5db",borderRadius:12,fontSize:13,color:"#111827",outline:"none",background:"#f9fafb",boxSizing:"border-box"}}/></div><div style={{fontSize:12,color:"#6b7280",fontWeight:500}}>Showing {(auditPage-1)*AUDIT_PAGE_SIZE + 1} - {Math.min(auditPage*AUDIT_PAGE_SIZE, filteredAuditLogs.length)} of {filteredAuditLogs.length} logs</div></div><div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:700}}><thead><tr style={{borderBottom:"2px solid #e5e7eb"}}>{["Action","Performed By","Target ID","Reason","Time"].map(h=><th key={h} style={{padding:"14px 16px",textAlign:"left",fontSize:11,fontWeight:700,color:"#6b7280",textTransform:"uppercase",letterSpacing:".05em"}}>{h}</th>)}</tr></thead><tbody>{paginatedAuditLogs.length===0?<tr><td colSpan={5} style={{textAlign:"center",padding:60,color:"#6b7280",fontSize:14}}>No audit logs found</td></tr>:paginatedAuditLogs.map(log=>(<tr key={log.id} className="row-hover" style={{borderBottom:"1px solid #f3f4f6"}}><td style={{padding:"14px 16px"}}><span style={{fontFamily:"monospace",fontSize:12,fontWeight:600,padding:"4px 10px",borderRadius:8,background:"#f3f4f6",color:"#4b5563"}}>{log.action_type}</span></td><td style={{padding:"14px 16px",fontSize:13,color:"#111827",fontWeight:600}}>{log.admin_name}</td><td style={{padding:"14px 16px",fontSize:12,fontFamily:"monospace",color:"#6b7280",maxWidth:140}}><div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{log.target_id??"—"}</div></td><td style={{padding:"14px 16px",fontSize:13,color:"#4b5563",maxWidth:280}}><div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{log.reason??"—"}</div></td><td style={{padding:"14px 16px",fontSize:12,color:"#6b7280",whiteSpace:"nowrap"}}>{timeAgo(log.created_at)}</td></tr>))}</tbody></table></div>{totalAuditPages>1&&(<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:24,padding:"16px 0",borderTop:"1px solid #f3f4f6"}}><button onClick={()=>setAuditPage(p=>Math.max(1,p-1))} disabled={auditPage===1} style={{padding:"8px 16px",borderRadius:10,border:"1px solid #e5e7eb",background:auditPage===1?"#f9fafb":"#fff",color:auditPage===1?"#9ca3af":"#4b5563",fontSize:13,fontWeight:600,cursor:auditPage===1?"not-allowed":"pointer"}}>Previous</button><div style={{display:"flex",alignItems:"center",gap:4}}>{Array.from({length:Math.min(5,totalAuditPages)},(_,i)=>{let pNum=i+1;if(totalAuditPages>5&&auditPage>3){pNum=auditPage-2+i;if(pNum+2>totalAuditPages)pNum=totalAuditPages-4+i;}if(pNum<=0||pNum>totalAuditPages)return null;return(<button key={pNum} onClick={()=>setAuditPage(pNum)} style={{width:36,height:36,borderRadius:10,border:"none",background:auditPage===pNum?"#1c4532":"transparent",color:auditPage===pNum?"#fff":"#4b5563",fontSize:13,fontWeight:700,cursor:"pointer"}}>{pNum}</button>);})}</div><button onClick={()=>setAuditPage(p=>Math.min(totalAuditPages,p+1))} disabled={auditPage===totalAuditPages} style={{padding:"8px 16px",borderRadius:10,border:"1px solid #e5e7eb",background:auditPage===totalAuditPages?"#f9fafb":"#fff",color:auditPage===totalAuditPages?"#9ca3af":"#4b5563",fontSize:13,fontWeight:600,cursor:auditPage===totalAuditPages?"not-allowed":"pointer"}}>Next</button></div>)}</div>)}
