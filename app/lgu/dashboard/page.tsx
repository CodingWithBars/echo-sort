...
// FIXES:
//   1. collection_schedules 403 → SQL policy fix (see bottom of file comment)
//   2. Escalated report → violation insert now uses a valid enum value mapping
//   3. Notification panel on mobile → renders as bottom sheet, not absolute popup
// ─────────────────────────────────────────────────────────────────────────────
...
import {
  ...
  ClipboardList, TrendingUp, ShieldCheck, Recycle,
} from "lucide-react";
...
// ── FIX 2: Map free-text report types → valid violation_type enum values ──────
// citizen_reports.type is text (flexible); violations.type is a Postgres ENUM.
// If the report type already matches an enum value exactly, it passes through.
// Otherwise we fall back to the closest match or "Improper Disposal".
const VALID_VIOLATION_ENUM = new Set([
  "Improper Disposal","Open Burning","Littering",
  "Illegal Dumping","Mixed Waste","Overflowing Bin",
  "Prohibited Area Dumping","Hazardous Waste Mishandling",
]);
function toViolationEnum(reportType: string): string {
  const trimmed = (reportType ?? "").trim();
  if (VALID_VIOLATION_ENUM.has(trimmed)) return trimmed;
  // Case-insensitive fallback
  for (const v of VALID_VIOLATION_ENUM) {
  ...
  return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,padding:"15px 18px",borderBottom:`1px solid ${EM[100]}`,background:EM[50],borderRadius:"18px 18px 0 0",flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0,flex:1}}>
        {Icon && <div style={{width:36,height:36,borderRadius:10,background:EM[100],display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon size={17} color={EM[700]}/></div>}
      ...
      <button onClick={onClose} style={{width:32,height:32,borderRadius:9,border:`1px solid ${EM[200]}`,background:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><X size={14} color={EM[700]}/></button>
    </div>
  );
}
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {BROADCAST_TYPES.map(t=>(
            <button key={t.id} onClick={()=>setType(t.id)}
              style={{padding:"6px 12px",borderRadius:20,border:`1.5px solid ${type===t.id?EM[400]:EM[100]}`,background:type===t.id?EM[100]:"#fff",cursor:"pointer",fontSize:11,fontWeight:type===t.id?700:500,color:type===t.id?EM[800]:"#374151",display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {BROADCAST_TEMPLATES.map(s=>(
              <button key={s.id}
                ...
                style={{padding:"10px 12px",borderRadius:10,textAlign:"left",cursor:"pointer",width:"100%",border:`1.5px solid ${picked===s.id?EM[400]:EM[100]}`,background:picked===s.id?EM[50]:"#fff",display:"flex",alignItems:"center",gap:10}}>
...
function ReportModal({report,profile,onClose,onRefresh}:{report:CitizenReport;profile:LGUProfile;onClose:()=>void;onRefresh:()=>void}) {
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      ...
      // ── FIX 2: Escalate → insert violation with correct enum value ────────
      if (newStatus === "Escalated" && report.reported_id) {
        // Map the free-text report type to a valid violation_type enum value
        const violationType = toViolationEnum(report.type);
        const violationDesc = [
        ...
        const { error: vErr } = await supabase.from("violations").insert({
          ...
          type:        violationType,   // ← guaranteed to be a valid enum value
        ...
        if (vErr) {
          ...
          setInsertError(`Violation insert failed: ${vErr.message} (type sent: "${violationType}")`);
        } else {
          // Notify the reported citizen
          await supabase.from("notifications").insert({
  ...
  return (
    <Modal onClose={onClose} wide>
      ...
      <div style={{padding:"18px 22px",display:"flex",flexDirection:"column",gap:16}}>
        ...
        {insertError && (
          <div style={{padding:"10px 14px",borderRadius:10,background:"#fef2f2",border:"1px solid #fecaca",display:"flex",alignItems:"center",gap:8}}>
            <AlertTriangle size={15} color="#dc2626"/><span style={{fontSize:12,color:"#991b1b"}}>{insertError}</span>
          </div>
        )}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {report.reported_id ? (
              <button
                ...
                disabled={saving}
                style={{padding:"12px 18px",borderRadius:10,background:"#dc2626",color:"#fff",border:"none",fontSize:13,fontWeight:800,cursor:saving?"not-allowed":"pointer",display:"flex",alignItems:"center",gap:8,opacity:saving?.6:1,boxShadow:"0 4px 16px rgba(220,38,38,.25)",transition:"all .15s"}}
              >
              ...
              <div style={{padding:"10px 14px",borderRadius:10,background:"#fef3c7",border:"1px solid #fde68a",fontSize:12,color:"#92400e",display:"flex",alignItems:"center",gap:8}}>
                ⚠ Cannot escalate — no reported citizen identified.
              </div>
            )}
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <button onClick={()=>updateStatus("Under Review")} disabled={saving||report.status==="Under Review"}
...
function ScheduleModal({profile,schedule,onClose,onRefresh}:{profile:LGUProfile;schedule?:Schedule;onClose:()=>void;onRefresh:()=>void}) {
  ...
  const save = async () => {
    ...
    if (schedule) {
      ...
      const { error } = await supabase.from("collection_schedules").insert(payload);
      opError = error;
    }
    if (opError) {
      console.error("collection_schedules save:", opError);
      setSaveError(`Save failed (${opError.code}): ${opError.message}`);
      setSaving(false);
      return;
    }
    await supabase.from("audit_logs").insert({
      ...
      target_id:   schedule?.id ?? "new",
      reason:      `${schedule ? "Updated" : "Created"} schedule "${label}" for Barangay ${profile.barangay}`,
    });
    setSaving(false);
    onRefresh();
    onClose();
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <div style={{padding:"18px 22px",display:"flex",flexDirection:"column",gap:14}}>
        {/* ── FIX 1 NOTICE: shown if save failed with 403 ── */}
        {saveError && (
          <div style={{padding:"10px 14px",borderRadius:10,background:"#fef2f2",border:"1px solid #fecaca",fontSize:12,color:"#991b1b"}}>
            <strong>Error:</strong> {saveError}
            {saveError.includes("42501") && (
              <div style={{marginTop:6,color:"#7f1d1d"}}>
                Run this SQL in Supabase to fix RLS: <br/>
                <code style={{fontFamily:"monospace",fontSize:11,background:"#fff",padding:"2px 6px",borderRadius:4}}>
                  CREATE POLICY "schedules: lgu manages own barangay" ON public.collection_schedules FOR ALL USING (public.is_lgu_for_barangay(barangay));
                </code>
              </div>
            )}
          </div>
...
// ── NOTIFICATION PANEL — FIX 3: mobile bottom sheet ──────────────────────────
// On mobile (< 640px): fixed bottom sheet anchored to screen bottom, full width.
// On desktop: absolute dropdown below the bell button.
...
function NotifPanel({notifs,onRead,onClose}:{notifs:DBNotif[];onRead:(id:string)=>void;onClose:()=>void}) {
  ...
  return (
    <>
      {/* ── FIX 3: backdrop only on mobile, closes panel ── */}
      <div
        onClick={onClose}
        style={{
        ...
        className="notif-backdrop-always"
      />
      <style>{`
        ...
        @media(max-width:639px){
          ...
          .notif-backdrop-always{ display:block!important; }
        }
        ...
        @media(min-width:640px){
          ...
          .notif-backdrop-always{ display:none!important; }
        }
      `}</style>
      <div
        ...
        style={{
          background:"#fff",
          border:`1.5px solid ${EM[100]}`,
          boxShadow:`0 -4px 40px rgba(6,78,59,.18)`,
      ...
      >
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 18px",borderBottom:`1px solid ${EM[100]}`,background:EM[50],flexShrink:0}}>
          <div style={{fontSize:14,fontWeight:800,color:EM[900]}}>
            Notifications
            {unread>0&&<span style={{fontSize:10,fontWeight:800,marginLeft:7,background:"#ef4444",color:"#fff",padding:"1px 7px",borderRadius:20}}>{unread}</span>}
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",display:"flex",padding:4}}><X size={16} color={EM[700]}/></button>
        </div>
        <div style={{overflowY:"auto",flex:1}}>
          {notifs.length===0 ? (
            <div style={{padding:36,textAlign:"center",color:"#9ca3af",fontSize:13}}>No notifications yet</div>
          ) : notifs.map(n=>(
...
export default function Page() {
  ...
  // No mousedown-outside listener needed anymore — the backdrop handles closing
  ...
  useEffect(()=>{
    ...
    const ch1=supabase.channel("lgu-cd").on("postgres_changes",{event:"INSERT",schema:"public",table:"citizen_details",filter:`barangay=eq.${profile.barangay}`},()=>{addN({type:"new_citizen",title:"New Citizen Registered",body:`A new resident has registered under Barangay ${profile.barangay}.`,created_at:new Date().toISOString()});fetchData();}).subscribe();
    const ch2=supabase.channel("lgu-viol").on("postgres_changes",{event:"INSERT",schema:"public",table:"violations",filter:`barangay=eq.${profile.barangay}`},(payload:any)=>{addN({type:"REPORT_RECEIVED",title:"New Violation Reported",body:`A violation (${payload.new?.type?.replace(/_/g," ")??"unknown"}) was reported in your barangay.`,created_at:new Date().toISOString()});fetchData();}).subscribe();
    const ch3=supabase.channel("lgu-rep").on("postgres_changes",{event:"INSERT",schema:"public",table:"citizen_reports",filter:`barangay=eq.${profile.barangay}`},(payload:any)=>{addN({type:"REPORT_RECEIVED",title:"New Citizen Report",body:`A citizen has filed a report (${payload.new?.type?.replace(/_/g," ")??"unknown"}) in your barangay.`,created_at:new Date().toISOString()});fetchData();}).subscribe();
    const ch4=supabase.channel("lgu-notifs").on("postgres_changes",{event:"INSERT",schema:"public",table:"notifications",filter:`user_id=eq.${profile.id}`},(payload:any)=>{setNotifs(p=>[payload.new as DBNotif,...p].slice(0,40));}).subscribe();
  ...
  const TABS=[
    {id:"citizens",   label:"Citizens",   count:citizens.length,    badge:0},
    {id:"violations", label:"Violations", count:violations.length,  badge:0},
    {id:"reports",    label:"Reports",    count:reports.length,     badge:pendingRep},
    {id:"schedules",  label:"Schedules",  count:schedules.length,   badge:0},
    {id:"broadcasts", label:"Broadcasts", count:broadcasts.length,  badge:0},
    {id:"overview",   label:"Overview",   count:null,               badge:0},
  ];
    ...
    <div className="flex h-screen w-full bg-[#F8FAFC] font-sans relative overflow-hidden text-slate-900">
      ...
      <aside className={`fixed inset-y-0 left-0 z-[2001] w-72 bg-white border-r border-slate-200 transform transition-transform duration-500 ease-in-out lg:translate-x-0 lg:static flex flex-col ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 px-4 space-y-1.5 mt-2 overflow-y-auto">
          ...
          {TABS.map(tab => {
            ...
            return (
              <button key={tab.id}
                onClick={() => { setActiveTab(tab.id as any); setIsSidebarOpen(false); setSearch(""); }}
                ...
                <div className="flex items-center gap-4">
                  ...
                  {tab.badge > 0 && (
                    <span className={`text-[9px] font-black px-1.5 py-0.5 rounded-full ${isActive ? "bg-white/20 text-white" : "bg-red-500 text-white"}`}>{tab.badge}</span>
                  )}
                </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full relative overflow-hidden bg-slate-50/50">
        ...
        <header className="h-20 bg-white/70 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-6 lg:px-10 shrink-0 z-[1002]">
          ...
          <div className="flex items-center gap-3">
            {/* ── Bell — notifRef wraps it so NotifPanel is positioned relative to it on desktop ── */}
            <div ref={notifRef} style={{position:"relative"}}>
              ...
              {/* NotifPanel now handles its own backdrop and positioning per breakpoint */}
              {notifOpen && <NotifPanel notifs={notifs} onRead={markRead} onClose={() => setNotifOpen(false)} />}
            </div>
            {/* Profile badge */}
            <div className="flex items-center gap-3 p-1.5 pr-4 rounded-xl border bg-white border-slate-200 shadow-sm">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center overflow-hidden border border-slate-100 bg-emerald-600">
                <span className="font-black text-white text-sm uppercase">{(profile?.full_name ?? "L").charAt(0)}</span>
              </div>
              <div className="text-left hidden md:block">
                <p className="text-[11px] font-bold text-slate-900 uppercase tracking-tight">{profile?.full_name ?? "LGU Official"}</p>
                <div className="flex items-center gap-1.5">
                  <ShieldCheck size={10} className="text-emerald-600" />
                  <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500">{profile?.position_title ?? "LGU Officer"}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div style={{maxWidth:1200,margin:"0 auto",padding:"24px 20px"}}>
            ...
            {/* Stat cards */}
            <div className="sgrid" style={{display:"grid",gap:14,marginBottom:24}}>
                ...
                <div style={{padding:22,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:18}}>
                  ...
                  <div style={{background:"#fffbeb",borderRadius:14,padding:20,border:"1px solid #fde68a"}}>
                    ...
                    {[
                      ...
                      {icon:"📅",title:"Schedule Reminders",   desc:"Edge Function cron: send COLLECTION_REMINDER notification 30 min before each schedule."},
                    ].map(f=>(
        ...
        <div className="fixed inset-0 z-[3000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="relative w-full max-w-sm bg-white rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-300 border border-slate-100">
            <div className="text-center">
              ...
              <p className="text-xs font-medium text-slate-500 mb-8 leading-relaxed">You are about to sign out of the LGU portal. All active sessions will be closed.</p>
...
/*
═══════════════════════════════════════════════════════════════════
FIX 1 — collection_schedules 403 (RLS policy missing for LGU)
═══════════════════════════════════════════════════════════════════

The migration SQL only added policies for ADMIN/SUPER_ADMIN to write
to collection_schedules. LGU needs its own INSERT/UPDATE/DELETE policy.

Run this once in Supabase SQL Editor:

  -- Allow LGU to manage schedules for their own barangay
  CREATE POLICY "schedules: lgu manages own barangay"
    ON public.collection_schedules
    FOR ALL
    USING (public.is_lgu_for_barangay(barangay))
    WITH CHECK (public.is_lgu_for_barangay(barangay));

  -- Also allow LGU to insert (created_by = their own id is enforced by the app)
  -- The USING clause above covers SELECT/UPDATE/DELETE.
  -- The WITH CHECK clause covers INSERT.

After running that SQL the 403 will disappear immediately.
═══════════════════════════════════════════════════════════════════
*/